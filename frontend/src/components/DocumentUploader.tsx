...
const DocumentUploader: React.FC = () => {
  ...
  const _uploadContent = async (presignedurl: RequestInfo | URL, type: any, content: any) => {
    fetch(presignedurl, {
      method: "PUT",
      body: content,
  ...
  const uploadFile = async () => {
    var query = url || searchQuery || selectedFile?.name;
    if (searchQuery) {
      query = `SEARCH:${searchQuery}`;
    }

    setButtonStatus("uploading");
    API.get("serverless-pdf-chat", "/generate_presigned_url", {
      ...
      if (response.type === "application/pdf") {
        _uploadContent(response.presignedurl, response.type, selectedFile);
      }
      if (response.type === "text/html" || response.type == "application/json") {
        const blobData = new Blob([response.content], {type: response.type});
        _uploadContent(response.presignedurl, response.type, blobData);
      }
    });
  };
  ...
  return (
  <div>
    <h2 className="text-2xl font-bold pb-4">Document Input</h2>
    <div className="flex justify-start space-x-4 mb-4">
      ...
      <label>
        <input
          ...
          name="inputType"
          value="search"
          checked={inputType === "search"}
          onChange={() => setInputType("search")}
        /> Search Query
      </label>
    </div>

    {(inputType === "file" && inputStatus === "idle") && (
  ...
  {inputType === "file" && inputStatus === "valid" && (
      <div className="flex items-center justify-center w-full">
        <div className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg bg-gray-50">
          <>
            ...
            <div className="flex flex-row items-center">
              {(buttonStatus === "ready" || buttonStatus === "uploading") && (
                <button
                  disabled={buttonStatus === "uploading"}
                  onClick={resetStatus}
                  type="button"
    ...
    {(inputType === "search" || inputType === "url") && (
    <button
      onClick={uploadFile}
      disabled={buttonStatus === "uploading"}
      className={`mt-4 py-2 px-4 rounded font-bold text-white ${buttonStatus !== "uploading" ? 'bg-blue-500 hover:bg-blue-700' : 'bg-gray-500'}`}>
      Submit
    </button>
    )}
  </div>
